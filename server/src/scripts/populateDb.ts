  ...
  try {
    ...
    for (const job of mockJobs) {
      await client.query(
        ...
        [job.id, job.employerId, job.title, job.salary, job.status, job.commute, job.description, job.notes, job.jobDescriptionLink]
      );
    }
    console.log(`Attempted to insert ${mockJobs.length} jobs (existing IDs skipped).`);
