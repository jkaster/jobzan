    ...
    try {
      ...
      if (response.status === 202 || response.status === 429 || response.status === 503) {
        ...
        if (retryCount >= maxRetries) {
          throw new Error(`Request failed after ${maxRetries} retries: ${response.statusText}`);
        }
        ...
        console.warn(`Retrying request to ${url} in ${finalDelay / 1000} seconds (attempt ${retryCount}/${maxRetries})...`);
      ...
      const jitter = Math.random() * 500; // Add random jitter up to 500ms
      const finalDelay = delay + jitter;
      console.warn(`Network error, retrying request to ${url} in ${finalDelay / 1000} seconds (attempt ${retryCount}/${maxRetries})...`);
      await new Promise(resolve => setTimeout(resolve, finalDelay));
    }
  }
